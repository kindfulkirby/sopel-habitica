...
def show_status(bot, trigger):
    ...
    if api_user is None:
        ...
        if api_key is None:
            user = requests.get("https://habitica.com/api/v2/members/" + api_user, headers=Common.auth)
        else:
...
def add_user(args, bot, trigger):
    if trigger.is_privmsg:
        user_id = Common.uuid_regex.findall(args[1])
        ...
        user = requests.get("https://habitica.com/api/v2/members/" + user_id, headers=Common.auth)
        name = user.json()["profile"]["name"]
...
def add_key(args, bot, trigger):
    if trigger.is_privmsg:
        user_key = Common.uuid_regex.findall(args[2])
